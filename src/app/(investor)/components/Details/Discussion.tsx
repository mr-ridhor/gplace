      ...
      <div className="w-full gap-y-2 md:grid-cols-2 text-[10px] lg:text-sm grid gap-x-4 ">
        ...
        <Card className="border col-span-1">
          ...
          <CardContent className="text-[10px] lg:text-sm w-full space-y-4  ">
            <div className="w-full  2xl:w-[80%] justify-between h-1/2 flex gap-x-1">
              ...
              <div className="space-y-2  w-[35%]">
                ...
                <div className="w-full items-center">
                  {Array.isArray(geography) ? (
                    geography
                      .reduce((result: any[], geo: any, index: number) => {
                        const chunkIndex = Math.floor(index / 2); // Create a new chunk every 2 items
                        if (!result[chunkIndex]) {
                          result[chunkIndex] = []; // Initialize new chunk
                        }
                        result[chunkIndex].push(geo);
                        return result;
                      }, [])
                      .map((chunk, rowIndex) => (
                        <div
                        ...
                        >
                          {chunk.map((geo: any, colIndex: number) => (
                            <div
                              key={colIndex}
                              className="flex px-2 w-max bg-[#F5E2B7] rounded-md items-center justify-center h-8"
                            >
          ...
          <div className="col-span-1">
            ...
            {editingState.offerPrice ? (
              ...
              />
                ...
                <div className="flex items-center gap-x-2" onDoubleClick={handleDoubleClick("offerPrice")}>

              <p >
                ${formatNumberWithCommas(`${values.offerPrice}`)}
              </p>
			  <PencilLine  className="" size={16}/>
