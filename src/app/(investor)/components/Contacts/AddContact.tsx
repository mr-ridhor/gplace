...
const AddContact: React.FC<Props> = ({ selectedItem, onClose }) => {
	...
	//     const response = await axios.post(`/api/investors/${investorId}/contact`, payload);

	//     // Dispatch an action to add the new contact to the Redux state
	...
	const onSubmit = async (data: contType) => {
		const payload = {
			_id: "", // Assuming this is generated by your backend
			name: data.name,
			surname: data.surname,
			email: data.email,
			phone: data.phone,
			title: data.title,
			contactType: data.contactType, // Assuming contactType comes from your form
		};
